...
Poder analisar um processo é uma das melhores formas de compreender a execução deste, quando não é possível aceder ao código fonte. Ao executar a monitorização o desempenho do(s) processo(s), bem como do próprio sistema diminui. Como a monitorização de rede não é excepção, este trabalho centra-se na forma de diminuir o impacto negativo da monitorização de rede de um processo.
A monitorização da comunicação de um processo com o exterior é complicada, pois ou se conhece \textit{à priori} quais os portos que irão ser utilizados, ou é necessário capturar todo o tráfego e apenas no final filtrar quais os pacotes referentes ao processo. A forma de minimizar este impacto negativo foi criar uma forma de filtragem dinâmica através da monitorização das chamadas ao sistema de comunicação com o exterior. Assim é possível criar um repositório com os endereços e portos utilizados por um processo, e capturar apenas o trafego que flue nestes. Deste modo é possível capturar apenas os dados referente às comunicações com o exterior efectuadas por um processo. Consegue-se diminuir o número de dados transferidos entre o núcleo de sistema e a aplicação monitorizadora, o que permite um menor número de trocas de contexto, que é um dos principais problemas na monitorização. 
