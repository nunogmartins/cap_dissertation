...
\subsection{Firewall do Linux}
\label{sub:linux_firewall}

Existe um sistema de \textit{firewall} no \textit{Linux}, que gere o fluxo de dados de e para o exterior através de politicas de controlo. Este sistema é o \texit{NetFilter} e está dentro do núcleo do \textit{Linux}. A gestão do fluxo de dados é efectuada através regras, estas podem ser indicadas em qualquer momento ao \textit{NetFilter} para poderem ser aplicadas. Estas regras são baseadas na definição do tráfego, ou seja, se é de entrada ou de saída ou de redireccionamento, entre outros parâmetros.
