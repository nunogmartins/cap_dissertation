...
Um dos componentes do \textit{netfilter} designado por \textit{connection tracking}, efectua o acompanhamento do início ao fim das ligações, para o que necessita conhecer os diferentes protocolos utilizados.
Tal como o \textit{netfilter}, este componente pode igualmente ser controlada pelo administrador, através da aplicação \textit{conntrack}.
Como se depreende este componente, \textit{connection tracking}, permite efectuar uma gestão mais eficiente e inteligente das conexões, sendo geralmente referido como \textit{firewall} com estado, é também utilizada para efectuar \textit{NAT} (\textit{Network Address Translation}) sobre as interfaces de rede.
...
O filtro de pacotes ao aceder ao repositório e decidir quais os pacotes a capturar, origina a filtragem dinâmica orientada ao processo.0
