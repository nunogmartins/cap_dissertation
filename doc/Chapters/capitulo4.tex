...
Como a obtenção dos pacotes por estes canais, é efectuada antes da recepção destes pelos processos, existe a possibilidade do \textit{NetFilter} bloquear a recepção de alguns pacotes, impedindo as aplicações de os receberem, não obstante o canal já os ter obtido para análise.
...
Um dos componentes do \textit{netfilter} designado por \textit{connection tracking}, cuja funcionalidade permite que se efectue o acompanhamento das ligações desde o seu inicio até ao seu \textit{terminus}.
Para realizar este acompanhamento, este componente necessita conhecer os diferentes protocolos utilizados.
Tal como o \textit{nefilter} pode ser controlado por um programa em nível utilizador, este componente \textit{connection tracking}, pode igualmente ser controlado pelo administrador, através do \textit{conntrack}.
...
Relativamente à componente instrumentação das chamadas ao sistema (ou outras funções contidas no sistema de rede), esta actualiza a componente repositório de dados, onde é mantido o estado das interacções via rede do(s) processo(s) alvo, com as efectuadas pelo processo.
...
Caso seja capturado indica que este pertence ao processo alvo, caso contrário irá ser sujeito a um processo de avaliação, baseado nas restantes regras de filtragem.
