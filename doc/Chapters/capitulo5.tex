...
Optou-se então, por excluir esta alternativa em virtude da criação e destruição de processos que podessem apresentar custos elevados.
...
No núcleo existem subsistemas que implementaram tabelas de dispersão com base em \textit{arrays} e listas, permitindo deste modo utilizar as vantagens desta estrutura de dados.
...
Esta alteração permitiu adicionar uma nova funcionalidade, não obstante de se assistir a uma ligeira sobrecarga no sistema de monitorização, independentemente da mesma estar ou não activa.
