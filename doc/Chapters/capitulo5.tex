...
O recurso a um mapa de \textit{bits}, permitiria de um modo bastante rápido e com uma reduzida utilização de memória, conhecer se um determinado porto estaria em utilização, por um determinado processo.
...
O repositório criado através de uma árvore \textit{Red and Black} contém vários elementos com uma estrutura bem definida, que contém obrigatoriamente um elemento \textit{rb\_node}, para possibilitar a manipulação da árvore e um outro elemento, comparativo, utilizado como chave.
...
A \textit{API}, não obstante reduzir o desempenho, devido à necessidade de chamar os métodos específicos ao repositório, permite a sua substituição, sem que se verifique a modificação do código.
