...
Com a utilização de redes de 100 ou 1000 \textit{Mbps} o número de interrupções que são efectuadas a cada pacote que chega a interface de rede é muito elevado, levando a que para cada pacote a ser transferido entre o \textit{buffer} da interface de rede e a memória central seja necessário uma interrupção ao \textit{cpu}.
...
Assim, foi criada uma nova \textit{API} (\textit{Application Program Interface}) de atendimento de interrupções do processador, oriundos das interfaces de rede, com o objectivo de aumentar o desempenho da utilização de rede de elevado débito.
...
De modo a tirar partido das arquitecturas \textit{multi-core}, é necessário que o controlador e interfaces de rede, os \textit{buffers}, e os controladores de \textit{DMA} (\textit{Direct Memory Access}), sejam modificados de forma a conhecerem esta estrutura.
