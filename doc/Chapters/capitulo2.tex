...
Outros mecanismos em especial para a implementação no \textit{Linux}, podem influenciar o desempenho deste sistema, como a seguir se discute.
...
Apesar de melhorar significativamente o número de cópias de dados, como os blocos de memória para a introdução dos pacotes é efectuada de modo estático, existem apenas um número fixo de pacotes que podem ser mantidos.
Deste modo, se o ritmo de escrita dos pacotes na zona de memória for superior ao ritmo de leitura pela ferramenta, podem existir perdas de pacotes, devido à inexistência de espaço para guardar os novos, enquanto os anteriores pacotes não forem consumidos.
...
Este módulo difere na abordagem utilizada no \textit{PACKET\_MMAP}, porquanto nesta a memória é mapeada entre a ferramenta e o controlador da interface, enquanto que no \textit{PACKET\_MMAP} a memória é mapeada entre a ferramenta e um \textit{buffer} interno ao núcleo do sistema, mas externo ao controlador da interface.
...
Relativamente ao \textit{software}, são conhecidos esforços na utilização de técnicas de partilha de \textit{buffers}, como o \textit{mmap}, de modo a reduzir o número de cópias de dados entre as aplicações e o núcleo de sistema como visto.
...
\label{sub:napi}
Com a utilização de redes a 100 ou 1000 \textit{Mbps} o número de interrupções, por unidade de tempo, que são efectuadas é muito elevado.
...
O \textit{Tracepoints} e o \textit{Linux Kernel Markers} fazem parte da instrumentação estática do núcleo \textit{Linux}.
Estes mecanismos apesar de fazerem parte da instrumentação estática do núcleo, são activados e desactivados dinamicamente através das ferramentas em nível utilizador.
Estas ferramentas tiram partido dos mecanismos de instrumentação estática e dinâmica do núcleo, de modo a obter os dados relativos às funções para efectuar o traço de execução.
Os dados provenientes da monitorização, no \textit{LTTng} são recolhidos através do sistema de ficheiros virtual \textit{RelayFs}, enquanto que para o \textit{Ltt} poderiam ser recolhidos através do \textit{daemon} \textit{KLogd}.

Para além do \textit{daemon} em nível utilizador, existem outras ferramentas pertencentes ao \textit{LTT} e \textit{LTTng}, que permitem efectuar análises aos dados recolhidos durante a monitorização.
...
O \textit{OProfile} não permite apenas efectuar o perfil de utilização do núcleo \textit{Linux}, pode igualmente criar o perfil de utilização dos processos de nível utilizador, que estão em execução na máquina.
...
O \textit{OProfile} regista os valores do \textit{program counter} (registo do processador que contém a próxima instrução a executar) e o instante em que este foi actualizado, assim com estes dados e conhecendo as zonas de memória consegue efectuar o perfil de execução.
...
Nas subsecções anteriores foram apresentadas ferramentas de monitorização e os seus mecanismos para lidar com a monitorização.
...
Em ambos é possível definir novos pontos, funções ou eventos a serem monitorizados, as funcionalidades já existentes nestas ferramentas.
...
\begin{itemize}
 \item \textbf{KProbe - } Utilizando o \textit{KProbe} pode analisar-se uma instrução e, para isso tem de ser indicada a distância desta ao inicio da função instrumentada.
...
Para efectuar esta operação é utilizada a técnica do trampolim~\cite{Hollingsworth94dynamicprogram}, uma vez que uma função pode terminar em diversos pontos.
...
Para ultrapassar esta limitação, novas ferramentas foram a ser desenvolvidas.
