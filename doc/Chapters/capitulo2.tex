...
Através da função \textit{pcap\_compile} presente na biblioteca, irá traduzir o filtro e o optimizará para o conjunto de instruções do \textit{BPF}, sendo posteriormente aplicado no núcleo através da função \textit{setfilter}.
...
Desta forma, caso tenha sido aplicado um filtro no canal, menos pacotes têm de ser processados e passados para o monitor, o que pode reduzir significativamente a sobrecarga exercida.
...
Para além destas modificações no \textit{PACKET\_MMAP}, o núcleo de sistema de operação \textit{Linux} na sua versão 2.6, passou a contar com a nova \textit{API} de rede (\textit{NAPI}), apresentada na secção \ref{sub:napi}.
