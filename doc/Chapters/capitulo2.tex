...
A arquitectura do \textit{Packet Capture} (\textit{PCap}) divide-se em duas partes: a biblioteca em nível utilizador, e a implementação do \textit{Linux Socket Filtering} no núcleo de sistema.
...
Se a ferramenta que utiliza a biblioteca necessitar filtrar os pacotes com base nos metadados destes, deve especificar um filtro utilizando para isso as instruções pertencentes à biblioteca \textit{PCap} que as irá traduzir e optimizar para a linguagem (\textit{BPF}) de modo a ser aplicada no núcleo.
 O filtro é aplicado ao canal de monitorização criado, através da chamada ao sistema \textit{setsockopt}, passando em argumentos o canal e o filtro a ser executado a cada pacote.
...
As ferramentas que utilizam esta biblioteca, não necessitam de conhecer estes mecanismos internos, sendo-lhes facultado uma \textit{API} que lhes permite abstrair das especificidades da arquitectura onde estão a operar.
