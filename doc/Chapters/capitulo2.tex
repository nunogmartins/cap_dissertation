...
Quando os filtros são demasiado complexos, a componente de filtragem do núcleo tem de transferir o pacote para o monitor em nível utilizador, de modo a poder ser aplicar o filtro.
Estes fitros analisam um nível mais abstracto da informação, presente no \textit{payload} dos pacotes, ou a relacionam com outra informação, para obterem apenas os dados relevantes.
...
Como este \textit{socket} comunica, directamente, com a interface de rede permite que sejam capturados logo os pacotes que nela circulam, efectuando assim a monitorização dos pacotes de rede.
...
Um dos parâmetros necessários à monitorização, é a interface sobre a qual irá ser efectuada, assim de modo a monitorizar todas as interfaces simultâneamente, foi criado um modo especial, o \textit{any}.
...
Por isso, de modo a minimizar a sobrecarga e capturar apenas os pacotes relevantes para as análises, pode ser necessário filtrar os pacotes não relevantes.
A biblioteca permite efectuar diversas formas de filtragem, quer em nível utilizador, para filtros mais complexos, quer no núcleo com filtros apenas baseados nos metadados dos pacotes.
Se a filtragem for efectuada exclusivamente em nível utilizador, é necessário aplicar um filtro que indique que todos os pacotes devem ser capturados.
Se a ferramenta que utiliza a biblioteca necessitar filtrar os pacotes com base nos metadados destes, deverá especificar um filtro utilizando para isso as instruções pertencentes à biblioteca \textit{PCap}.
...
Através da função \textit{pcap\_compile} presente na biblioteca, irá traduzir e optimizar o filtro para o conjunto de instruções do \textit{BPF}, sendo posteriormente aplicado no núcleo através da função \textit{setfilter}.
Esta função utiliza a chamada ao sistema \textit{setsockopt}, passando como argumentos o canal e o filtro a ser executado, para que este seja avaliado a cada pacote recebido ou enviado através da interface de rede.
...
É possível obter estatisticas, relacionadas com a captura e filtragem de pacotes, estas são também obtidas através de \textit{ioctls} efectuadas sobre o canal de recepção da monitorização.
...
Esta estrutura dispõe de toda a informação sobre o pacote, não só o \textit{payload} recebido pelas aplicações, como também os metadados, por isso é utilizada nos sistemas de monitorização.

As ferramentas que utilizam esta biblioteca, não necessitam de conhecer estes mecanismos internos, sendo-lhes facultado uma \textit{API} que lhes permite abstrair das especificidades da arquitectura onde estão a executar.
...
Como o anteriormente referido, o \textit{LSF} utiliza um \textit{socket} da família \textit{AF\_PACKET} que comunica directa com o controlador da interface de rede, possibilitando a captura dos diversos pacotes ainda antes destes poderem ser filtrados pela \textit{firewall}, presente no núcleo de sistema.
...
O \textit{LSF} é derivado do \textit{Berkeley Packet Filtering} (\textit{BPF}) que é o \textit{standard} de \textit{de facto} para a criação de filtros para a captura de pacotes de rede, no núcleo de sistema.
Este sistema de captura e filtragem permite aos administradores, definir e afectar filtros ao \textit{socket} da familia \textit{AF\_PACKET}.
Os filtros definidos são descritos numa linguagem simples, de forma a efectuarem com celeridade a selecção dos pacotes a capturar, rejeitando todos os outros.
...
Este filtro é como um programa que irá ser executado, passo a passo, sobre o pacote, recebido ou enviado, através de uma máquina virtual desenhada para o efeito.
No retorno da execução deste programa / filtro, é decidido se o pacote deve, ou não ser capturado, continuando o seu percurso pelo sistema de monitorização, até ao monitor em nível utilizador, que em geral é uma ferramenta que utiliza a biblioteca \textit{PCap}.
...
Neste caso, cabe à ferramenta monitora definir explicitamente que deseja utilizar este modo de captura e, de modo a atingir este objectivo tem de pedir ao núcleo, uma região de memória, sendo esta, posteriormente indicada como argumento de configuração do \textit{socket}.
...
Apesar da \textit{API} da biblioteca \textit{PCap} ser homogénea em diferentes plataformas (\textit{Windows}, \textit{Linux}, \textit{FreeBSD}, etc), o desempenho desta pode variar.
...
Dependendo das análises a serem efectuadas, pode ser interessante e vantajoso desligar o modo permiscuo da interface de rede a monitorizar.
Assim, caso o este modo esteja desligado, um menor número de pacotes serão processados e transferidos para o monitor, o que reduzirá significativamente a sobrecarga exercida.
...
Para diminuir o número de cópias de dados, entre o núcleo e o monitor da monitorização em nível utilizador, foi utilizada a técnica de partilha de um \textit{buffer}.
...
Esta técnica consiste na utilização de um bloco de memória pertencente ao processo monitor, de modo a que quando seja necessário transferir um pacote do núcleo para a ferramenta, não se efectue a cópia de memória onde reside o pacote para a do processo.
...
Apesar de melhorar significativamente o número de cópias de dados, como os blocos de memória para a introdução dos pacotes é efectuada de modo estático, existem apenas um número fixo de pacotes que podem ser alocados.
Deste modo, se o ritmo de escrita dos pacotes na zona de memória for superior ao ritmo de leitura, podem existir perdas de pacotes, devido à inexistência de espaço para alocar novos, enquanto os anteriores pacotes não forem consumidos.
...
Igualmente no \textit{hardware}, tem-se assistido a uma evolução, no sentido de reduzir as interrupções efectuadas ao \textit{cpu}, adicionando nas interfaces de rede processadores dedicados a certas funcionalidades presentes no núcleo, de modo a libertá-lo da execução destas tarefas.
Estas tarefas podem ser de monitorização, colocação de estampilha temporal, reagrupar segmentos, filtragem de pacotes, etc.
Não só têm sido adicionadas mais funcionalidades, como também tem sido adicionados multiplas filas de espera, para que estas possam tirar partido das arquitecturas de multiplos cores e das memórias \textit{cache} residentes nestes processadores.
...
De modo a tirar partido das arquitecturas \textit{multi-core}, é necessário que o controlador e interfaces de rede, os \textit{buffers}, e os controladores de \textit{DMA}, sejam modificados de forma a conhecerem esta estrutura.
...
Assim nesta secção são apresentados ferramentas e mecanismos de monitorização do núcleo.

A utilização de sistemas de monitorização ao nível do núcleo, permite efectuar a monitorização genérica dos processos, de forma não intrusiva para as aplicações.
...
De entre os mecanismos e ferramentas de monitorização dinâmica analisadas é possível agrupá-las em duas categorias: eventos pré-definidos e instrumentação dinâmica.
Na categoria instrumentação dinâmica existem dois mecanismos mais relevantes: o \textit{KProbes} e o \textit{Linux Kernel Stace Tracer}, enquanto que para os eventos pré-definidos foram verificados o \textit{Linux Trace Toolkit} e o \textit{OProfile}.
