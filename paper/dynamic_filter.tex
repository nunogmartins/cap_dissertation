...
%frase nova % 
 ...
 O processo de modificação ou afectação de um filtro, é iniciado pela indicação das regras de filtragem, seguido de optimização, paragem do sistema e drenagem do canal de filtragem, e por fim afectação do novo filtro ao canal.
%end of frase nova
...
A função de filtragem  implementada neste sistema assenta no estado do processo alvo, mantido pelos módulos anteriormente descritos.
 ...
 Se existir é reportado ao \textit{LSF} para continuar a análise dos filtros definidos estáticamente.
 Caso contrário é reportado para ser rejeitado e aí termina o processamento deste pacote.
 Desta forma mantem-se a compatibilidade e continua-se a tirar partido dos beneficios da utilização do \textit{Linux Socket Filter}.
